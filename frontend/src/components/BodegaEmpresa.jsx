...
import { Boxes, Building2, PackagePlus, ArrowRightLeft, AlertTriangle, Search, Plus, Save, Warehouse, History, PackageCheck } from 'lucide-react';
...
export default function BodegaEmpresa({ user, onBack }) {
  ...
  const [movForm, setMovForm] = useState({ bodega_id: '', producto_id: '', tipo: 'Entrada', cantidad: '', costo_unitario: '', documento: '', contraparte: '', observaciones: '' });
  const [transferForm, setTransferForm] = useState({ origen: '', destino: '', producto_id: '', cantidad: '', observaciones: '' });

  const load = async () => {
    if (!empresa) return;
    setLoading(true); setMessage('');
    const [b, p, m, c, o] = await Promise.all([
    ...
    const error = b.error || p.error || m.error || c.error || o.error;
    if (error) setMessage(`No fue posible cargar Bodega: ${error.message}`);
    else { setBodegas(b.data || []); setProductos(p.data || []); setMovimientos(m.data || []); setCentros(c.data || []); setObras(o.data || []); }
    setLoading(false);
  };
  ...
  const registerMovement = async event => {
    ...
    const payload = { empresa, ...movForm, bodega_id: Number(movForm.bodega_id), producto_id: Number(movForm.producto_id), cantidad: Number(movForm.cantidad), costo_unitario: Number(movForm.costo_unitario || product?.costo_referencia || 0), responsable: user?.nombre || user?.usuario || user?.correo || 'Usuario Obraxis' };
    ...
    setMovForm({ bodega_id: '', producto_id: '', tipo: 'Entrada', cantidad: '', costo_unitario: '', documento: '', contraparte: '', observaciones: '' }); setMessage('Movimiento registrado con trazabilidad.'); await load();
  };
  ...
  const tabs = [{ id: 'stock', label: 'Stock', icon: Boxes }, { id: 'movimientos', label: 'Movimientos', icon: PackagePlus }, { id: 'transferencias', label: 'Transferencias', icon: ArrowRightLeft }, { id: 'maestros', label: 'Configuración', icon: Warehouse }];
  return <div className="space-y-5">
    ...
    {tab === 'movimientos' && <div className="grid gap-4 xl:grid-cols-[380px_1fr]"><form onSubmit={registerMovement} className="space-y-3 rounded-2xl border bg-white p-5"><h3 className="font-black">Registrar movimiento</h3><select required value={movForm.bodega_id} onChange={e => setMovForm({...movForm,bodega_id:e.target.value})} className={field}><option value="">Bodega</option>{bodegas.filter(x=>x.activo).map(b=><option key={b.id} value={b.id}>{b.codigo} · {b.nombre}</option>)}</select><select required value={movForm.producto_id} onChange={e => setMovForm({...movForm,producto_id:e.target.value})} className={field}><option value="">Producto</option>{productos.filter(x=>x.activo).map(p=><option key={p.id} value={p.id}>{p.codigo} · {p.nombre}</option>)}</select><select value={movForm.tipo} onChange={e=>setMovForm({...movForm,tipo:e.target.value})} className={field}>{['Entrada','Salida','Ajuste +','Ajuste -'].map(x=><option key={x}>{x}</option>)}</select><div className="grid grid-cols-2 gap-2"><input required type="number" min="0.0001" step="any" placeholder="Cantidad" value={movForm.cantidad} onChange={e=>setMovForm({...movForm,cantidad:e.target.value})} className={field}/><input type="number" min="0" step="any" placeholder="Costo unitario" value={movForm.costo_unitario} onChange={e=>setMovForm({...movForm,costo_unitario:e.target.value})} className={field}/></div><input placeholder="Guía, factura u OC" value={movForm.documento} onChange={e=>setMovForm({...movForm,documento:e.target.value})} className={field}/><input placeholder="Proveedor / destinatario" value={movForm.contraparte} onChange={e=>setMovForm({...movForm,contraparte:e.target.value})} className={field}/><textarea placeholder="Observaciones" value={movForm.observaciones} onChange={e=>setMovForm({...movForm,observaciones:e.target.value})} className={field}/><button className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 py-3 text-xs font-black text-white"><Save className="h-4 w-4"/>Registrar</button></form><MovementTable rows={movimientos} bodegas={bodegas} productos={productos}/></div>}
