...
export default function BodegaEmpresa({ user, onBack }) {
  ...
  const stockRows = useMemo(() => {
    const map = new Map();
    movimientos.filter(m => (m.estado_validacion || 'Validado') === 'Validado').forEach(m => {
  ...
  const registerMovement = async event => {
    ...
    setMovForm({ bodega_id: '', producto_id: '', tipo: 'Entrada', cantidad: '', costo_unitario: '', documento: '', contraparte: '', observaciones: '', dte_documento_id: '', generar_guia: false }); setMessage(dteId ? 'Recepción registrada pendiente de validación. El stock aún no fue modificado.' : 'Movimiento registrado con trazabilidad.'); await load();
  };

  const selectGuide = guide => { const expectedType = guide.direccion === 'Recibido' ? 'Entrada' : 'Salida'; setMovForm(prev => ({ ...prev, tipo: expectedType, dte_documento_id: guide.id, documento: `Guía N° ${guide.folio || 'borrador'}`, contraparte: guide.razon_social_contraparte || '', costo_unitario: prev.costo_unitario })); setTab('movimientos'); setMessage('Guía seleccionada. Completa el producto y la cantidad física recibida o despachada.'); };
...
function MovementTable({ rows, bodegas, productos }) { const bMap=new Map(bodegas.map(x=>[x.id,x])); const pMap=new Map(productos.map(x=>[x.id,x])); const validate=async(m,state)=>{const {error}=await supabase.from('bodega_movimientos').update({estado_validacion:state,validado_por:'Usuario Obraxis',validado_at:new Date().toISOString()}).eq('id',m.id);if(error)alert(error.message);else window.location.reload()}; return <div className="overflow-x-auto rounded-2xl border bg-white"><table className="w-full min-w-[980px] text-left text-xs"><thead className="bg-slate-100 text-[10px] uppercase text-slate-500"><tr><th className="p-3">Fecha</th><th className="p-3">Movimiento</th><th className="p-3">Producto</th><th className="p-3">Bodega</th><th className="p-3">Cantidad</th><th className="p-3">Documento</th><th className="p-3">Validación</th><th className="p-3">Responsable</th></tr></thead><tbody className="divide-y">{rows.map(m=>{const state=m.estado_validacion||'Validado';return <tr key={m.id}><td className="p-3">{new Date(m.fecha).toLocaleString('es-CL')}</td><td className="p-3 font-bold">{m.tipo}</td><td className="p-3">{pMap.get(m.producto_id)?.nombre || 'Producto'}</td><td className="p-3">{bMap.get(m.bodega_id)?.nombre || 'Bodega'}</td><td className={`p-3 font-mono font-black ${positiveTypes.has(m.tipo)?'text-emerald-700':'text-rose-700'}`}>{positiveTypes.has(m.tipo)?'+':'-'}{qty(m.cantidad)}</td><td className="p-3">{m.documento || '—'}</td><td className="p-3">{state==='Pendiente'?<div className="flex gap-1"><button onClick={()=>validate(m,'Validado')} className="rounded-lg bg-emerald-700 px-2 py-1 text-[10px] font-black text-white">Validar</button><button onClick={()=>validate(m,'Rechazado')} className="rounded-lg bg-rose-50 px-2 py-1 text-[10px] font-black text-rose-700">Rechazar</button></div>:<span className={`rounded-full px-2 py-1 text-[10px] font-black ${state==='Validado'?'bg-emerald-50 text-emerald-700':'bg-rose-50 text-rose-700'}`}>{state}</span>}</td><td className="p-3">{m.responsable || '—'}</td></tr>})}</tbody></table>{!rows.length&&<p className="p-8 text-center text-xs text-slate-500">Aún no hay movimientos.</p>}</div>; }
