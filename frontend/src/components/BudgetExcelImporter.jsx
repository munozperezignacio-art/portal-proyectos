...
export default function BudgetExcelImporter({ presupuestoId, projectCurrency = 'CLP', onImported }) {
  ...
  const readBc3File = async event => {
    ...
    try {
      ...
      const { data: existingRows, error: existingError } = await supabase.from('presupuestos_items').select('codigo').eq('presupuesto_id', Number(presupuestoId));
      if (existingError) throw existingError;
      const existingCodes = new Set((existingRows || []).map(row => normalize(row.codigo).toLowerCase()));
      const conflictingCodes = parsed.items.map(item => normalize(item.codigo)).filter(code => existingCodes.has(code.toLowerCase()));
      setImportMode(existingCodes.size ? 'replace' : 'append');
      ...
      setPreview({ partidas:parsed.items, recursos:parsed.resources, globalCosts:parsed.globalCosts, existingCount:existingCodes.size, conflictingCodes, fileName:file.name, origin:'PRESTO_BC3', warnings:parsed.warnings, metadata:parsed.metadata });
    } catch (error) { setErrors([error.message]); }
    event.target.value = '';
  };
  ...
  const importBudget = async () => {
    ...
    const rpcName = isBc3 && importMode === 'replace' ? 'reemplazar_presupuesto_bc3_completo' : isBc3 ? 'importar_presupuesto_bc3_completo' : 'importar_presupuesto_jerarquico';
