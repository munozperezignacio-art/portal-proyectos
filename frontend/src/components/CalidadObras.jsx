...
export default function CalidadObras({ user, onBack, obraInicial = '', embedded = false, obraPerfil = null }) {
  ...
  return <div className="space-y-5">
    ...
    {!obraNombre ? <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center text-sm text-slate-500">Selecciona una obra para comenzar su control de calidad.</div> : loading ? <div className="p-10 text-center text-sm text-slate-500">Cargando calidad de obra…</div> : <>
      ...
      </div><div className="grid gap-4 lg:grid-cols-2"><section className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="mb-3 text-sm font-black text-slate-800">Partidas bloqueadas o por inspeccionar</h3>{rdisPendientes.length ? <div className="space-y-2">{rdisPendientes.slice(0,6).map(r => <RdiRow key={r.id} rdi={r} onStatus={updateStatus} />)}</div> : <Empty text="No hay RDI pendientes en esta obra." />}</section><section className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="mb-3 text-sm font-black text-slate-800">No conformidades prioritarias</h3>{ncsAbiertas.length ? <div className="space-y-2">{ncsAbiertas.slice(0,6).map(n => <NcRow key={n.id} nc={n} onStatus={updateStatus} />)}</div> : <Empty text="No hay no conformidades abiertas." />}</section></div><section className="rounded-2xl border border-violet-200 bg-violet-50 p-5"><div className="flex items-center gap-2"><Repeat2 className="h-4 w-4 text-violet-800"/><h3 className="text-sm font-black text-violet-950">Recurrencia y focos sistémicos</h3></div>{recurrence.length ? <div className="mt-3 grid gap-2 md:grid-cols-2">{recurrence.slice(0,8).map(item => <button key={`${item.partida}-${item.causa}`} type="button" onClick={()=>setTab('nc')} className="rounded-xl border border-violet-100 bg-white p-3 text-left"><p className="text-xs font-black text-slate-900">{item.partida}</p><p className="mt-1 text-[10px] font-semibold text-slate-500">{item.causa}</p><p className="mt-2 text-[10px] font-black text-violet-800">{item.total} NC · {item.abiertas} abierta(s)</p></button>)}</div> : <p className="mt-3 text-xs text-slate-600">Aún no existen combinaciones repetidas de partida y categoría de causa.</p>}</section></div>}
