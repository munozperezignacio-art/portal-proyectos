...
export default function CalidadObras({ user, onBack, obraInicial = '', embedded = false, obraPerfil = null }) {
  ...
  const updateStatus = async (table, id, estado) => {
    ...
    try {
      ...
      if (nc && estado === 'Cerrada' && nc.estado !== 'Verificada') { setMessage('Una NC solo puede cerrarse después de verificar la eficacia de la acción correctiva.'); return; }
...
function NcRow({ nc, onStatus, onUpdate, full }) {
  ...
  const next = nc.estado === 'Abierta' ? 'En corrección' : nc.estado === 'En corrección' ? 'Verificada' : nc.estado === 'Verificada' ? 'Cerrada' : '';
