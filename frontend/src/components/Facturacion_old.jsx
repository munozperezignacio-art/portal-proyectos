...
export default function Facturacion({ user, companyBranding, onBack }) {
  ...
  // -------------------------------------------------------------
  // ESTADOS DE CENTROS DE GESTI├ôN
  // -------------------------------------------------------------
  ...
  // -------------------------------------------------------------
  // ESTADOS DE ├ôRDENES DE COMPRA (OC)
  // -------------------------------------------------------------
  ...
  // Formulario nueva venta (Emisi├│n DTE)
  ...
  // Referencias en DTE
  const [dteRefTipo, setDteRefTipo] = useState(''); // ej. 801 (OC), 52 (Gu├¡a)
  const [dteRefFolio, setDteRefFolio] = useState('');
  const [dteRefFecha, setDteRefFecha] = useState('');

  // Formulario ingreso nueva compra (Conciliaci├│n)
  ...
  // -------------------------------------------------------------
  // CONTROLADORES DE ACCIONES: CENTROS DE GESTI├ôN
  // -------------------------------------------------------------
  const handleSaveCentro = async () => {
    if (!/^\d{3}$/.test(centroCodigo)) {
      setErrorMsg("El c├│digo del Centro de Gesti├│n debe tener exactamente 3 d├¡gitos num├®ricos (ej: 001, 102, 999).");
      return;
    }
    if (!centroNombre.trim()) {
      setErrorMsg("Debes ingresar un nombre para el Centro de Gesti├│n.");
      return;
    }
    setLoading(true);
    setErrorMsg('');
    ...
    try {
      ...
      setSuccessMsg(`Centro de Gesti├│n "${centroCodigo} - ${centroNombre}" creado con ├®xito.`);
      setCentroCodigo('');
      setCentroNombre('');
      ...
      setErrorMsg(err.message.includes('unique_empresa_codigo') 
        ? "El c├│digo ingresado ya est├í registrado para otro Centro de Gesti├│n." 
        : err.message
      );
    } finally {
      setLoading(false);
  ...
  const handleDeleteCentro = async (id) => {
    if (!confirm("┬┐Est├í seguro de eliminar este Centro de Gesti├│n? Las ├│rdenes y facturas asociadas perder├ín su v├¡nculo.")) return;
    try {
      const { error } = await supabase
        .from('facturacion_centros_gestion')
        .delete()
      ...
      alert("Error eliminando Centro de Gesti├│n: " + err.message);
    }
  };

  // -------------------------------------------------------------
  // CONTROLADORES DE ACCIONES: CONFIGURACI├ôN SII
  // -------------------------------------------------------------
  const handleSaveConfigSii = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setSuccessMsg("Configuraci├│n del SII guardada con ├®xito en la base de datos.");
      fetchConfigSii();
      setCertFile(null);
      setCertPassword('');
    } catch (err) {
      setErrorMsg("Error guardando configuraci├│n: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTestSiiConnection = async () => {
    ...
    setTestingSii(false);
    setSiiStatusMsg("Ô£à SII API responde OK. Canal de Comunicaci├│n de Certificaci├│n disponible. Token obtenido con ├®xito mediante firma del certificado.");
  };
  ...
  const handleUploadCAF = async () => {
    ...
    if (isNaN(desdeNum) || isNaN(hastaNum) || desdeNum >= hastaNum) {
      setErrorMsg("Rangos de folios no v├ílidos.");
      return;
    }
    setLoading(true);
    setErrorMsg('');
    ...
    try {
      ...
      if (error) throw error;
      setSuccessMsg("Folios CAF autorizados y cargados con ├®xito.");
      setCafDesde('');
      setCafHasta('');
      fetchFolios();
    } catch (err) {
  ...
  // -------------------------------------------------------------
  // CONTROLADORES DE ACCIONES: ├ôRDENES DE COMPRA (OC)
  // -------------------------------------------------------------
  const handleSaveOC = async () => {
    ...
    if (!ocCentroGestion) {
      setErrorMsg("Debes asignar la Orden de Compra a un Centro de Gesti├│n.");
      return;
    }
    setLoading(true);
    setErrorMsg('');
    ...
    try {
      ...
      setSuccessMsg(`Orden de Compra N┬║ ${folioSecuencial} creada con ├®xito.`);
  ...
  // -------------------------------------------------------------
  // CONTROLADORES DE ACCIONES: EMISI├ôN DE DTE (VENTAS)
  // -------------------------------------------------------------
  const handleEmitirDTE = async () => {
    ...
    if (!dteCentroGestion) {
      setErrorMsg("Asigna el documento a un Centro de Gesti├│n.");
      return;
    }
...
<DTE version="1.0">
  ...
  <Signature>
    ...
    <SignatureValue>Adf5H3jKlOp98S... (Firma del XML SII)</SignatureValue>
      ...
      setSuccessMsg(`Documento emitido con ├®xito. Folio N┬║ ${folioADoc} registrado y aceptado por el SII (Track ID: ${trackIdSimulado}).`);
  ...
  // -------------------------------------------------------------
  // CONTROLADORES DE ACCIONES: INGRESO DE COMPRA (CONCILIACI├ôN OC)
  // -------------------------------------------------------------
  const handleIngresarCompra = async () => {
    ...
    try {
      ...
      // Si vincul├│ una Orden de Compra, marcarla como Facturada en la BD
      ...
      setSuccessMsg(`Factura de Compra Folio N┬║ ${compFolio} de ${compNombreEmisor} guardada y conciliada.`);
  ...
  // -------------------------------------------------------------
  // C├üLCULOS TRIBUTARIOS DE DASHBOARD
  // -------------------------------------------------------------
  ...
  // F29 Estimado = IVA D├®bito (Ventas) - IVA Cr├®dito (Compras)
  const ivaEstimadoF29 = totalVentasIva - totalComprasIva;

  // Distribuci├│n de costos por centro de gesti├│n
  ...
  const getDteTypeName = (typeCode) => {
    switch (parseInt(typeCode)) {
      ...
      case 52: return "Gu├¡a de Despacho";
    ...
    <div className="space-y-6">
      ...
      {errorMsg && (
        <div className="bg-red-50 border border-red-200 text-red-800 p-4 rounded-2xl text-xs font-semibold flex items-center gap-2 animate-shake">
          ...
          <span>{errorMsg}</span>
          <button onClick={() => setErrorMsg('')} className="ml-auto text-red-400 hover:text-red-700">Ô£ò</button>
        </div>
      )}
      {successMsg && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 p-4 rounded-2xl text-xs font-semibold flex items-center gap-2 animate-in fade-in duration-200">
          ...
          <button onClick={() => setSuccessMsg('')} className="ml-auto text-emerald-400 hover:text-emerald-700">Ô£ò</button>
        </div>
      )}
        ...
        <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-semibold">Visualiza la posici├│n del impuesto mensual y facturaci├│n.</p>
            </div>
            <div className="flex items-center gap-2.5 w-full sm:w-auto shrink-0">
              <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Centro de Gesti├│n:</label>
              <select
                value={filtroCentro}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white border border-slate-250 rounded-3xl p-6 shadow-xs flex items-start gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[9.5px] text-slate-455 font-bold uppercase tracking-wider">IVA D├®bito: {formatCLP(totalVentasIva)}</p>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-250 rounded-3xl p-6 shadow-xs flex items-start gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[9.5px] text-slate-455 font-bold uppercase tracking-wider">IVA Cr├®dito: {formatCLP(totalComprasIva)}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-5 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4">
              <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider border-b pb-2">
                ­ƒôé Egresos de Caja por Centro de Gesti├│n
              </h4>
            ...
            <div className="lg:col-span-7 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4">
              <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider border-b pb-2">
                ­ƒôä ├Ültimos Documentos Emitidos / Recibidos
              </h4>
      ...
      {activeTab === 'centros' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-200">
          ...
          <div className="lg:col-span-4 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4 h-fit">
            <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider border-b pb-2">
              Ô×ò Crear Centro de Gesti├│n
            </h4>
            
            <div className="space-y-3 text-xs font-semibold text-slate-700">
              <div className="space-y-1">
                <label className="block text-[9px] font-bold uppercase text-slate-450">C├│digo del Centro (3 D├¡gitos)</label>
                <input
                  type="text"
                  maxLength={3}
                  value={centroCodigo}
              ...
              <div className="space-y-1">
                <label className="block text-[9px] font-bold uppercase text-slate-450">Nombre del Centro de Gesti├│n</label>
                <input
                  type="text"
                  value={centroNombre}
              ...
              <div className="space-y-1">
                <label className="block text-[9px] font-bold uppercase text-slate-450">Descripci├│n / Observaciones</label>
                <textarea
                  rows={3}
          ...
          <div className="lg:col-span-8 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4">
            <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider border-b pb-2">
              ­ƒôï Listado de Centros de Gesti├│n Activos
            </h4>
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-150">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-550 font-bold text-[9px] uppercase tracking-wider">
                    <th className="p-3 w-20">C├│digo</th>
                    <th className="p-3">Nombre del Centro</th>
                    <th className="p-3">Descripci├│n</th>
                    <th className="p-3 w-16 text-center">Acci├│n</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-150 text-slate-700 font-medium">
                  ...
                  {centros.length === 0 && (
                    <tr>
                      <td colSpan={4} className="py-8 text-center text-xs text-slate-450 font-bold uppercase">
                        No hay Centros de Gesti├│n registrados en Supabase.
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      {activeTab === 'ocs' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-800">├ôrdenes de Compra a Proveedores</h3>
              <p className="text-[10px] text-slate-500 font-semibold">Genera OCs formales y contr├│lalas.</p>
            </div>
            <button
              onClick={() => {
                setErrorMsg('');
          ...
          <div className="bg-white border border-slate-250 rounded-3xl p-6 shadow-xs">
            <div className="overflow-x-auto rounded-xl border border-slate-150">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-550 font-bold text-[9px] uppercase tracking-wider">
                    <th className="p-3 w-20">N┬║ OC</th>
                    <th className="p-3">Proveedor</th>
                    <th className="p-3">Centro Costos</th>
                    <th className="p-3 text-right">Monto Total</th>
                    <th className="p-3 text-center">Estado</th>
                    <th className="p-3 w-28 text-center">Acci├│n</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-150 text-slate-700 font-medium">
                  {ocsFiltradas.map((oc, idx) => {
                    const centroName = centros.find(c => c.id === oc.centro_gestion_id)?.nombre || 'Sin asignaci├│n';
                    return (
                      <tr key={idx} className="hover:bg-slate-50/50">
                        <td className="p-3 font-extrabold text-slate-850">N┬║ {oc.numero}</td>
                        <td className="p-3">
                  ...
                  {ocsFiltradas.length === 0 && (
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-xs text-slate-450 font-bold uppercase">
                        No hay ├ôrdenes de Compra registradas en este periodo.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          {/* VISOR DE IMPRESI├ôN DE ORDEN DE COMPRA SELECCIONADA */}
          {selectedOC && (
            <div className="bg-white border border-slate-250 rounded-3xl p-8 shadow-md max-w-3xl mx-auto space-y-6 relative animate-in fade-in duration-200">
              ...
              <div className="flex justify-between items-start border-b border-slate-200 pb-4">
                ...
                <div className="text-right space-y-1">
                  ...
                  <p className="text-xs font-black text-slate-800 mt-1">N┬║ {selectedOC.numero}</p>
              ...
              <div className="grid grid-cols-2 gap-6 bg-slate-50 border border-slate-200 rounded-2xl p-4 text-xs">
                <div>
                  <h4 className="text-[9px] font-black uppercase text-slate-450 mb-1">­ƒæ¿ÔÇì­ƒÆ╝ Proveedor:</h4>
                ...
                <div>
                  <h4 className="text-[9px] font-black uppercase text-slate-450 mb-1">­ƒôé Centro de Costos:</h4>
              ...
              {/* Bot├│n para Simular Env├¡o de Impresi├│n */}
              <div className="flex justify-end gap-2 border-t pt-4">
                <button
                  onClick={() => alert("Impresi├│n enviada a la cola del sistema de faena...")}
      ...
      {activeTab === 'emitir' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-200">
          ...
          <div className="lg:col-span-5 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4 h-fit">
            ...
            <div className="space-y-3 text-xs font-semibold text-slate-700">
              <div className="space-y-1">
                ...
                >
                  <option value={33}>Factura Electr├│nica (Afecta)</option>
                  ...
                  <option value={61}>Nota de Cr├®dito</option>
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <label className="block text-[9px] font-bold uppercase text-slate-455">Centro de Gesti├│n</label>
                  <select
              ...
              <div className="space-y-1">
                <label className="block text-[9px] font-bold uppercase text-slate-450">Raz├│n Social Receptor</label>
                <input
                  type="text"
              ...
              {/* Referencias Oficiales del Est├índar SII */}
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
                <h5 className="text-[9px] font-black uppercase text-slate-500 border-b pb-1">
                  ­ƒöù Documento de Referencia (Opcional - Est├índar SII)
                </h5>
                <div className="grid grid-cols-3 gap-2">
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      <option value="52">Gu├¡a Despacho</option>
          ...
          {/* Panel Derecho: Detalle de ├ìtems e Importes */}
          <div className="lg:col-span-7 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4">
            <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider border-b pb-2 flex justify-between items-center">
              <span>­ƒøá´©Å Detalle de ├ìtems del Documento</span>
              <button
              ...
              {dteItems.map((item, idx) => (
                <div key={idx} className="flex gap-2.5 items-end text-xs bg-slate-50 p-2.5 border border-slate-150 rounded-xl relative group">
                  <div className="flex-1 space-y-0.5">
                    <label className="text-[8px] font-bold uppercase text-slate-450">Descripci├│n</label>
                    <input
                      type="text"
            ...
            <div className="flex justify-between items-end border-t border-slate-100 pt-4 text-xs font-semibold text-slate-700">
              <div className="text-[10px] text-slate-450 max-w-[200px]">
                Ôä╣´©Å Facturas afectas llevan 19% de IVA en Chile. Boletas pueden llevar exento.
              </div>
      ...
      {activeTab === 'registro' && (
        <div className="space-y-6 animate-in fade-in duration-200">
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-150 text-slate-700 font-medium">
                  {docsFiltrados.filter(doc => doc.direccion_flujo === tipoFlujo).map((doc, idx) => {
                    const centroName = centros.find(c => c.id === doc.centro_gestion_id)?.nombre || 'Sin asignaci├│n';
                    return (
                      <tr key={idx} className="hover:bg-slate-50/50">
                        <td className="p-3 font-extrabold text-slate-850">N┬║ {doc.folio}</td>
                  ...
                  {docsFiltrados.filter(doc => doc.direccion_flujo === tipoFlujo).length === 0 && (
                    <tr>
                      <td colSpan={8} className="py-8 text-center text-xs text-slate-450 font-bold uppercase">
                        No hay documentos registrados en esta pesta├▒a.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          {/* VISOR DE FACTURA ELECTR├ôNICA (PDF O XML) */}
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
              <div className="bg-white border border-slate-250 rounded-3xl p-6 shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto relative animate-in zoom-in-95 duration-200">
                ...
                {showXMLViewer ? (
                  ...
                  <div className="space-y-6">
                    {/* Borde cl├ísico verde de Facturas SII de Chile */}
                    <div className="border-4 border-emerald-700 p-6 space-y-6 rounded-xl">
                      
                      {/* Cabecera Cl├ísica SII */}
                      <div className="flex justify-between items-start">
                        <div className="space-y-1">
                          <h2 className="text-sm font-black text-emerald-800 uppercase">{configSii.razon_social || user.empresa}</h2>
                          <p className="text-[10px] text-slate-500 font-extrabold uppercase">Giro: {configSii.giro || 'Construcci├│n / Ingenier├¡a'}</p>
                        ...
                        <div className="border-4 border-red-600 rounded-lg p-3 text-center w-52 shrink-0 space-y-1">
                          ...
                          <p className="text-red-600 text-[11px] font-black uppercase">N┬║ {selectedDTE.folio}</p>
                      ...
                      <div className="grid grid-cols-2 gap-4 border border-slate-200 rounded-lg p-3 text-[10px] text-slate-650 font-bold bg-slate-50/50">
                        <div>
                          <p className="text-slate-450 text-[8px] uppercase">Fecha Emisi├│n:</p>
                        ...
                        <div>
                          ...
                          <p className="text-slate-450 text-[8px] uppercase mt-2">Centro de Gesti├│n:</p>
                        ...
                        <table className="w-full text-left text-[10px] border-collapse">
                          <thead>
                            <tr className="bg-slate-100 border-b border-slate-200 text-slate-600 font-bold text-[8px] uppercase tracking-wider">
                              <th className="p-2">Descripci├│n Producto/Servicio</th>
                      ...
                      {/* Totales y Timbre Electr├│nico */}
                      <div className="flex justify-between items-end gap-6 text-[10px]">
                        
                        {/* Timbre Electr├│nico SII Bidimensional Simulado */}
                        <div className="border border-slate-300 p-2 text-center space-y-1.5 w-60 rounded bg-white">
                          <div className="bg-slate-900 w-full h-16 flex items-center justify-center text-white text-[7px] font-mono leading-none">
                            [ C├ôDIGO PDF417 TIMBRE ELECTR├ôNICO SII ]
                          </div>
                          <p className="text-[6.5px] text-slate-500 font-bold uppercase tracking-tighter">Timbre Electr├│nico S.I.I. - Res. 80 del 2014 - Verifique documento en www.sii.cl</p>
                        </div>
          ...
          {/* MODAL INGRESO DE COMPRA (CONCILIACI├ôN OC) */}
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
              <div className="bg-white border border-slate-250 rounded-3xl p-6 shadow-2xl max-w-lg w-full relative animate-in zoom-in-95 duration-200">
                ...
                <div className="space-y-3 text-xs font-semibold text-slate-700">
                  ...
                  <div className="space-y-1">
                    <label className="block text-[9px] font-bold uppercase text-slate-450">Raz├│n Social Proveedor</label>
                    <input
                      type="text"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="block text-[9px] font-bold uppercase text-slate-450">Centro de Gesti├│n (Costos)</label>
                      <select
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        {ordenesCompra.filter(o => o.estado !== 'Facturada').map(o => (
                          <option key={o.id} value={o.id}>OC N┬║ {o.numero} ({o.proveedor_nombre}) - {formatCLP(o.monto_total)}</option>
                        ))}
                      </select>
                    </div>
                  </div>
      ...
      {activeTab === 'folios' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-200">
          ...
          <div className="lg:col-span-5 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4 h-fit">
            <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider border-b pb-2">
              ­ƒôÑ Autorizar y Cargar Folios CAF (XML)
            </h4>
            ...
            <div className="space-y-3 text-xs font-semibold text-slate-700">
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full border border-slate-250 rounded-xl p-2 text-xs font-bold text-slate-800 bg-white cursor-pointer"
                >
                  <option value={33}>Factura Electr├│nica (33)</option>
                  <option value={34}>Factura Exenta (34)</option>
                  <option value={39}>Boleta Electr├│nica (39)</option>
                  <option value={52}>Gu├¡a de Despacho (52)</option>
                  <option value={61}>Nota de Cr├®dito (61)</option>
                </select>
              </div>
          ...
          <div className="lg:col-span-7 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4">
            <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider border-b pb-2">
              ­ƒôè Disponibilidad de Folios por Tipo DTE
            </h4>
                ...
                return (
                  <div key={idx} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-2 text-xs">
                    ...
                    <div className="flex justify-between text-[10px] text-slate-500 font-bold uppercase">
                      ...
                      <span>Siguiente folio libre: N┬║ {folio.actual} ({pctConsumo}% consumido)</span>
                    </div>
                  </div>
                );
              })}
      ...
      {activeTab === 'config' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-200">
          ...
          <div className="lg:col-span-7 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-4">
            <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider border-b pb-2">
              ­ƒÅó Datos Tributarios del Emisor
            </h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold text-slate-700">
              ...
              <div className="space-y-1">
                <label className="block text-[9px] font-bold uppercase text-slate-450">Raz├│n Social</label>
                <input
                  type="text"
              ...
              <div className="space-y-1 sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="ej: Obras de ingenier├¡a civil, edificaci├│n, movimientos de tierra"
                  className="w-full border border-slate-250 rounded-xl p-2.5 text-xs text-slate-800 bg-white"
                />
              </div>
              <div className="space-y-1">
                <label className="block text-[9px] font-bold uppercase text-slate-450">Direcci├│n Casa Matriz</label>
                <input
                  type="text"
          ...
          <div className="lg:col-span-5 bg-white border border-slate-250 rounded-3xl p-6 shadow-xs space-y-5 h-fit">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3 text-xs font-semibold text-slate-700">
                ...
                <div className="space-y-1">
                  <label className="block text-[9px] font-bold uppercase text-slate-450">Contrase├▒a de Clave Privada</label>
                  <input
                    type="password"
                    value={certPassword}
            ...
            <div className="border-t border-slate-100 pt-4 space-y-3">
              <h4 className="text-[10px] text-slate-500 font-extrabold uppercase tracking-wider flex items-center justify-between">
                <span>­ƒôÂ Estado del Servidor del SII</span>
                <select
                ...
                >
                  <option value="Certificaci├│n">Certificaci├│n (Pruebas)</option>
                  <option value="Producci├│n">Producci├│n (Real)</option>
                </select>
              </h4>
