...
export default function FloatingOX({user,obra,onNavigate}){
 const[open,setOpen]=useState(false);
 useEffect(()=>setOpen(false),[obra?.nombre]);
 ...
 if(!obra?.nombre||typeof document==='undefined')return null;
 ...
 return createPortal(<>
  ...
  <aside aria-hidden={!open} className={`fixed inset-y-0 right-0 z-[70] flex w-full max-w-[540px] flex-col border-l border-slate-200 bg-slate-50 shadow-2xl transition-transform duration-300 ${open?'translate-x-0':'translate-x-full'}`}>
   <header className="flex items-center gap-3 border-b border-slate-200 bg-white px-5 py-4"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-slate-950 text-white shadow-lg"><OXIcon className="h-8 w-8"/></span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h2 className="text-lg font-black text-slate-950">OX</h2><span className="flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-black uppercase text-emerald-800"><ShieldCheck className="h-3 w-3"/>Solo lectura</span></div><p className="truncate text-xs text-slate-500">Asistente de {obra.nombre}</p></div><button type="button" onClick={()=>setOpen(false)} aria-label="Cerrar OX" className="rounded-full bg-slate-100 p-2 text-slate-600 hover:bg-slate-200"><X className="h-5 w-5"/></button></header>
   <div className="min-h-0 flex-1 overflow-y-auto p-4"><CopilotoObra user={user} obra={obra} onNavigate={navigate} compact/></div>
  </aside>
