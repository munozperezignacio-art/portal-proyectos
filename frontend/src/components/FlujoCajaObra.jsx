...
export default function FlujoCajaObra({ obra, user, partidas = [], costos = [], liquidaciones = [], avances = [] }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [budgetResult, paymentsResult] = await Promise.all([
        supabase.from('obra_presupuestos').select('flujo_caja_ajustes').eq('obra_nombre', obra.nombre).maybeSingle(),
  ...
  return <div className="space-y-4"><section className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="flex gap-3"><div className="rounded-xl bg-emerald-700 p-2.5 text-white"><WalletCards className="h-5 w-5" /></div><div><h3 className="text-sm font-black text-emerald-950">Flujo de caja de la obra</h3><p className="mt-1 text-[11px] text-emerald-900">Proyección mensual basada en presupuesto y programación; lo real se alimenta de costos y Estados de Pago aprobados.</p></div></div><span className={`rounded-xl px-3 py-2 text-xs font-black ${totals.flujo >= 0 ? 'bg-white text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>Saldo proyectado: {money(totals.flujo)}</span></div></section>
