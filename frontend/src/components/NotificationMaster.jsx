...
const emptyForm = {
  ...
  obra_nombre: '', roles: [], usuarios: [], correos: [''], canal_email: true,
...
export default function NotificationMaster({ user, obras = [], roles = [] }) {
  ...
  const load = async () => {
    ...
    if (ruleResult.error && !missing) {
      setMessage(/permission denied|row-level security/i.test(ruleResult.error.message || '')
        ? 'El Master ya está protegido por empresa. Para administrarlo debes iniciar sesión mediante Supabase Auth; la sesión heredada no puede acceder a estas reglas seguras.'
  ...
  const openEdit = (rule) => {
    ...
    setForm({
      ...
      obra_nombre: rule.obra_nombre || '', roles: normalizeArray(rule.destinatarios_roles),
  ...
  const save = async (event) => {
    ...
    const payload = {
      ...
      descripcion: form.descripcion, obra_nombre: form.obra_nombre || null, destinatarios_roles: form.roles,
  ...
  const filtered = rules.filter(rule => {
    ...
    return (!q || `${rule.nombre} ${rule.modulo} ${rule.descripcion}`.toLowerCase().includes(q))
      ...
      && (filter.work === 'Todas' || (filter.work === 'Global' ? !rule.obra_nombre : rule.obra_nombre === filter.work))
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-950 to-blue-950 p-5 text-white shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex gap-3"><div className="rounded-xl bg-white/10 p-3"><Bell className="h-6 w-6" /></div><div>
            <h3 className="text-lg font-black">Master de notificaciones</h3>
      ...
      {schemaMissing && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs font-semibold text-amber-900">Falta habilitar el Master de notificaciones en Supabase. Ejecuta <b>schema_notificaciones_master.sql</b> y actualiza.</div>}
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        {loading ? <div className="p-8 text-center text-xs text-slate-500">Cargando reglas...</div> : !filtered.length ? <div className="p-10 text-center"><Bell className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-2 text-sm font-bold text-slate-600">No hay reglas configuradas</p><p className="text-xs text-slate-400">Crea una regla o carga las recomendaciones base.</p></div> :
          <div className="divide-y divide-slate-100">{filtered.map(rule => <div key={rule.id} className="grid gap-3 p-4 lg:grid-cols-[1.5fr_1fr_1fr_auto] lg:items-center">
            ...
            <div className="text-xs"><div className="flex items-center gap-1 font-bold text-slate-700"><Settings2 className="h-3.5 w-3.5" /> {rule.obra_nombre || 'Todas las obras'}</div><div className="mt-1 text-[10px] text-slate-500">{normalizeArray(rule.destinatarios_roles).length ? normalizeArray(rule.destinatarios_roles).join(', ') : 'Sin roles asignados'}</div></div>
      ...
      {modal && <div className="fixed inset-0 z-[9999] flex items-center justify-center overflow-y-auto bg-slate-950/65 p-3 backdrop-blur-sm"><form onSubmit={save} className="my-auto max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl sm:p-7">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <label><span className="text-[10px] font-black uppercase text-slate-500">Alcance de obra</span><select value={form.obra_nombre} onChange={e => setForm({ ...form, obra_nombre: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-xs"><option value="">Todas las obras</option>{obras.map(o => <option key={o.nombre}>{o.nombre}</option>)}</select></label>
