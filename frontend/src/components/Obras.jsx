...
function Obras({ user, onBack, initialObraName, companyBranding }) {
  ...
  const [obraDetailsForm, setObraDetailsForm] = useState({ especialidad: '', cliente: '', cliente_email: '', cliente_telefono: '', ubicacion: '', admin_contrato: '', oficina_tecnica: '', prevencionista: '', estado: 'Activa', fecha_termino_real: '', motivo_cierre: '' });
  ...
  const openObraDetails = () => {
    setObraDetailsForm({ especialidad: selectedObra?.tipo || '', cliente: selectedObra?.cliente || '', cliente_email: selectedObra?.cliente_email || '', cliente_telefono: selectedObra?.cliente_telefono || '', ubicacion: selectedObra?.ubicacion || '', admin_contrato: selectedObra?.admin_contrato || '', oficina_tecnica: selectedObra?.oficina_tecnica || '', prevencionista: selectedObra?.prevencionista || '', estado: selectedObra?.estado || 'Activa', fecha_termino_real: toDateKey(selectedObra?.fecha_termino_real), motivo_cierre: selectedObra?.motivo_cierre || '' });
  ...
  const saveObraDetails = async (event) => {
    ...
    try {
      const payload = { tipo: obraDetailsForm.especialidad.trim() || 'Construcción General', cliente: obraDetailsForm.cliente.trim() || null, cliente_email: obraDetailsForm.cliente_email.trim() || null, cliente_telefono: obraDetailsForm.cliente_telefono.trim() || null, ubicacion: obraDetailsForm.ubicacion.trim() || null, admin_contrato: obraDetailsForm.admin_contrato.trim() || null, oficina_tecnica: obraDetailsForm.oficina_tecnica.trim() || null, prevencionista: obraDetailsForm.prevencionista.trim() || null, estado: obraDetailsForm.estado, fecha_termino_real: obraDetailsForm.fecha_termino_real || null, motivo_cierre: obraDetailsForm.motivo_cierre.trim() || null };
  ...
  const openIncidentFollowUp = (response) => {
    ...
    setIncidentFollowUpForm({
      ...
      clasificacion_evento: followUp.clasificacion_evento || 'Por determinar',
      potencial_gravedad: followUp.potencial_gravedad || 'Medio',
      metodologia_investigacion: followUp.metodologia_investigacion || '5 porqués',
      causa_inmediata: followUp.causa_inmediata || '',
      causa_raiz: followUp.causa_raiz || '',
      riesgo_matriz_referencia: followUp.riesgo_matriz_referencia || '',
      medidas_correctivas: followUp.medidas_correctivas || '',
      acciones: Array.isArray(followUp.acciones) ? followUp.acciones : [],
      verificacion_eficacia: followUp.verificacion_eficacia || 'Pendiente',
      evidencia_cierre: followUp.evidencia_cierre || '',
  ...
  const addIncidentAction = () => {
    setIncidentFollowUpForm(current => ({
      ...current,
      acciones: [...(current.acciones || []), { id: crypto.randomUUID(), descripcion: '', tipo: 'Correctiva', responsable: '', fecha_compromiso: '', estado: 'Pendiente' }]
    }));
  };

  const updateIncidentAction = (id, field, value) => {
    setIncidentFollowUpForm(current => ({ ...current, acciones: (current.acciones || []).map(action => action.id === id ? { ...action, [field]: value } : action) }));
  };

  const removeIncidentAction = (id) => {
    setIncidentFollowUpForm(current => ({ ...current, acciones: (current.acciones || []).filter(action => action.id !== id) }));
  };
  ...
  const saveIncidentFollowUp = async (event) => {
    ...
    const actions = incidentFollowUpForm.acciones || [];
    if (incidentFollowUpForm.investigacion_requerida && (!incidentFollowUpForm.causa_inmediata.trim() || !incidentFollowUpForm.causa_raiz.trim())) {
      setErrorMsg('La investigación requiere causa inmediata y causa raíz antes de guardar.');
      return;
    }
    if (actions.some(action => !action.descripcion.trim() || !action.responsable.trim() || !action.fecha_compromiso)) {
      setErrorMsg('Cada acción debe incluir descripción, responsable y fecha de compromiso.');
      return;
    }
    if (incidentFollowUpForm.estado_caso === 'Cerrado' && (actions.some(action => action.estado !== 'Verificada') || incidentFollowUpForm.verificacion_eficacia === 'Pendiente')) {
      setErrorMsg('Para cerrar el caso, todas las acciones deben estar verificadas y su eficacia evaluada.');
      return;
    }
    const nextAnswers = {
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-start border-b border-slate-200 pb-4">
            <div>
              ...
              {(selectedObra.cliente || selectedObra.ubicacion) && <p className="mt-1 text-[11px] font-medium text-slate-500">{selectedObra.cliente ? `Mandante: ${selectedObra.cliente}` : ''}{selectedObra.cliente && selectedObra.ubicacion ? ' · ' : ''}{selectedObra.ubicacion || ''}</p>}
            </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex flex-wrap gap-1 bg-slate-100 p-1 rounded-xl text-[11px] font-bold">
                      {[
                        ...
                        { id: 'libro_obra', label: '📖 Libro de Obra' },
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-6">
                ...
                <div className="relative border-l-2 border-slate-300 ml-4 space-y-6 pl-6">
                  ...
                  {(() => {
                    ...
                    (libroObraBitacoraList || []).forEach(folio => {
                      if (bitacoraFilters.includes('todos') || bitacoraFilters.includes('libro_obra')) {
                        const rawDt = folio.fecha || (folio.created_at ? String(folio.created_at).substring(0, 10) : fechaInicioReal);
                        unifiedBitacoraEvents.push({ type: 'libro_obra', dateStr: rawDt, dateObj: new Date(rawDt + 'T12:00:00'), title: `Libro de Obra · ${folio.folio || 'Folio sin número'} · ${folio.asunto || 'Registro'}`, description: folio.detalle || 'Sin detalle informado.', author: folio.emisor || 'Libro de Obra', badge: `${folio.tipo || 'Registro'} · ${folio.estado || 'Abierto'}`, color: folio.estado === 'Cerrado' ? 'emerald' : 'blue' });
                      }
                    });
                    ...
                    (eventosBitacoraList || []).forEach(evento => {
                      ...
                      const color = evento.categoria === 'Estados de Pago' || evento.categoria === 'Libro de Obra' ? 'blue' : evento.categoria === 'Prevención' ? 'rose' : 'amber';
            ...
            return (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                const daysToFinish = fechaTerminoEstimada
                  ...
                  : null;
                const upcomingHorizon = addCalendarDays(fCorteStr, 14);
                const upcomingPartidas = targetPartidas.map(partida => {
                  const startDate = getPartidaScheduledStart(partida, fInicioObraDefault);
                  const reports = filteredAvances.filter(report => isMatchPartidaName(report.partida, partida.partida));
                  return {
                    partida: partida.partida,
                    startDate,
                    daysUntil: startDate ? Math.max(0, countChileanBusinessDays(fCorteStr, startDate)) : null,
                    hasProgress: reports.length > 0
                  };
                }).filter(partida => partida.startDate && partida.startDate > fCorteStr && partida.startDate <= upcomingHorizon && !partida.hasProgress)
          ...
          {showIncidentFollowUp && selectedPrevencionResponse && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center overflow-y-auto bg-slate-950/55 p-3 backdrop-blur-sm">
              <form onSubmit={saveIncidentFollowUp} className="w-full max-w-3xl rounded-3xl bg-white p-6 shadow-2xl">
                ...
                {incidentFollowUpForm.investigacion_requerida && <section className="mt-5 rounded-2xl border border-amber-200 bg-amber-50/60 p-4"><div className="mb-3"><p className="text-xs font-black uppercase tracking-wider text-amber-900">Investigación del evento</p><p className="mt-1 text-[10px] text-amber-800">Clasifica el evento, identifica causas y relaciónalo con la matriz de riesgos vigente.</p></div><div className="grid gap-3 md:grid-cols-3"><label className="text-xs font-bold text-slate-700">Clasificación<select value={incidentFollowUpForm.clasificacion_evento} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, clasificacion_evento: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2"><option>Por determinar</option><option>Incidente</option><option>Casi accidente</option><option>Accidente sin tiempo perdido</option><option>Accidente con tiempo perdido</option><option>Enfermedad profesional</option></select></label><label className="text-xs font-bold text-slate-700">Potencial de gravedad<select value={incidentFollowUpForm.potencial_gravedad} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, potencial_gravedad: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2"><option>Bajo</option><option>Medio</option><option>Alto</option><option>Crítico</option></select></label><label className="text-xs font-bold text-slate-700">Método<select value={incidentFollowUpForm.metodologia_investigacion} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, metodologia_investigacion: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2"><option>5 porqués</option><option>Árbol de causas</option><option>ICAM</option><option>Otro</option></select></label></div><div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-slate-700">Causa inmediata<textarea rows={2} value={incidentFollowUpForm.causa_inmediata} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, causa_inmediata: event.target.value })} placeholder="Acto o condición que desencadenó el evento" className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2" /></label><label className="text-xs font-bold text-slate-700">Causa raíz<textarea rows={2} value={incidentFollowUpForm.causa_raiz} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, causa_raiz: event.target.value })} placeholder="Falla organizacional o de control que permitió el evento" className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2" /></label></div><label className="mt-3 block text-xs font-bold text-slate-700">Referencia en matriz de riesgos<input value={incidentFollowUpForm.riesgo_matriz_referencia} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, riesgo_matriz_referencia: event.target.value })} placeholder="Código, tarea o peligro asociado" className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2" /></label></section>}
                <div className="mt-4 grid gap-4"><label className="text-xs font-bold text-slate-700">Diagnóstico / calificación médica<textarea rows={2} value={incidentFollowUpForm.diagnostico} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, diagnostico: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" /></label><label className="text-xs font-bold text-slate-700">Resumen de medidas correctivas y preventivas<textarea rows={3} value={incidentFollowUpForm.medidas_correctivas} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, medidas_correctivas: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" /></label><div className="grid gap-4 md:grid-cols-2"><label className="text-xs font-bold text-slate-700">Responsable del seguimiento<input value={incidentFollowUpForm.responsable_seguimiento} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, responsable_seguimiento: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" /></label><label className="text-xs font-bold text-slate-700">Próximo seguimiento<input type="date" value={incidentFollowUpForm.fecha_seguimiento} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, fecha_seguimiento: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" /></label></div></div>
                <section className="mt-5 rounded-2xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-slate-800">Plan de acciones</p><p className="mt-1 text-[10px] text-slate-500">Cada medida mantiene responsable, plazo y estado verificable.</p></div><button type="button" onClick={addIncidentAction} className="rounded-xl bg-slate-900 px-3 py-2 text-[10px] font-black text-white">+ Agregar acción</button></div><div className="mt-3 space-y-3">{(incidentFollowUpForm.acciones || []).map((action, index) => <div key={action.id} className="rounded-xl bg-slate-50 p-3"><div className="grid gap-2 md:grid-cols-12"><input value={action.descripcion} onChange={event => updateIncidentAction(action.id, 'descripcion', event.target.value)} placeholder={`Acción ${index + 1}`} className="rounded-lg border border-slate-300 px-3 py-2 text-xs md:col-span-4" /><select value={action.tipo} onChange={event => updateIncidentAction(action.id, 'tipo', event.target.value)} className="rounded-lg border border-slate-300 bg-white px-2 py-2 text-xs md:col-span-2"><option>Correctiva</option><option>Preventiva</option><option>Contención</option></select><input value={action.responsable} onChange={event => updateIncidentAction(action.id, 'responsable', event.target.value)} placeholder="Responsable" className="rounded-lg border border-slate-300 px-3 py-2 text-xs md:col-span-2" /><input type="date" value={action.fecha_compromiso} onChange={event => updateIncidentAction(action.id, 'fecha_compromiso', event.target.value)} className="rounded-lg border border-slate-300 px-2 py-2 text-xs md:col-span-2" /><select value={action.estado} onChange={event => updateIncidentAction(action.id, 'estado', event.target.value)} className="rounded-lg border border-slate-300 bg-white px-2 py-2 text-xs md:col-span-1"><option>Pendiente</option><option>En curso</option><option>Implementada</option><option>Verificada</option></select><button type="button" onClick={() => removeIncidentAction(action.id)} className="rounded-lg text-xs font-black text-rose-700 md:col-span-1">Quitar</button></div></div>)}{!(incidentFollowUpForm.acciones || []).length && <p className="rounded-xl bg-slate-50 p-3 text-center text-xs text-slate-500">Aún no hay acciones desglosadas.</p>}</div><div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-slate-700">Verificación de eficacia<select value={incidentFollowUpForm.verificacion_eficacia} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, verificacion_eficacia: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2"><option>Pendiente</option><option>Eficaz</option><option>Parcialmente eficaz</option><option>No eficaz</option></select></label><label className="text-xs font-bold text-slate-700">Evidencia de cierre<input value={incidentFollowUpForm.evidencia_cierre} onChange={event => setIncidentFollowUpForm({ ...incidentFollowUpForm, evidencia_cierre: event.target.value })} placeholder="Folio, documento o evidencia verificada" className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" /></label></div></section>
      ...
      {showObraDetails && selectedObra && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center overflow-y-auto bg-slate-900/60 p-3 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-start justify-between border-b border-slate-200 pb-3"><div><h3 className="text-base font-black text-slate-900">Ficha de Obra</h3><p className="mt-1 text-xs text-slate-500">Datos administrativos, responsables, contacto y ciclo de vida del proyecto.</p></div><button onClick={() => setShowObraDetails(false)} className="rounded-lg p-1 text-slate-500 hover:bg-slate-100">✕</button></div>
            <form onSubmit={saveObraDetails} className="space-y-4"><section className="space-y-3"><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Identificación y mandante</p><div className="grid gap-3 sm:grid-cols-2"><label className="block text-xs font-bold text-slate-700">Especialidad<input value={obraDetailsForm.especialidad} onChange={event => setObraDetailsForm({ ...obraDetailsForm, especialidad: event.target.value })} placeholder="Ej. Edificación, vialidad, montaje" className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Cliente / mandante<input value={obraDetailsForm.cliente} onChange={event => setObraDetailsForm({ ...obraDetailsForm, cliente: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Correo de contacto<input type="email" value={obraDetailsForm.cliente_email} onChange={event => setObraDetailsForm({ ...obraDetailsForm, cliente_email: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Teléfono de contacto<input value={obraDetailsForm.cliente_telefono} onChange={event => setObraDetailsForm({ ...obraDetailsForm, cliente_telefono: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label></div><label className="block text-xs font-bold text-slate-700">Ubicación<input value={obraDetailsForm.ubicacion} onChange={event => setObraDetailsForm({ ...obraDetailsForm, ubicacion: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label></section><section className="space-y-3 border-t border-slate-200 pt-4"><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Equipo responsable</p><div className="grid gap-3 sm:grid-cols-3"><label className="block text-xs font-bold text-slate-700">Administrador de contrato<input value={obraDetailsForm.admin_contrato} onChange={event => setObraDetailsForm({ ...obraDetailsForm, admin_contrato: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Oficina técnica<input value={obraDetailsForm.oficina_tecnica} onChange={event => setObraDetailsForm({ ...obraDetailsForm, oficina_tecnica: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Prevencionista<input value={obraDetailsForm.prevencionista} onChange={event => setObraDetailsForm({ ...obraDetailsForm, prevencionista: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label></div></section><section className="space-y-3 border-t border-slate-200 pt-4"><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Ciclo de vida</p><label className="block text-xs font-bold text-slate-700">Estatus de la obra<select value={obraDetailsForm.estado} onChange={event => setObraDetailsForm({ ...obraDetailsForm, estado: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm"><option>Activa</option><option>En pausa</option><option>En cierre</option><option>Terminada</option><option>Archivada</option></select></label>{['Terminada', 'Archivada'].includes(obraDetailsForm.estado) && <div className="space-y-3 rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-bold text-amber-900">Al cerrar, la obra saldrá de la vista operativa y quedará disponible en el historial.</p><label className="block text-xs font-bold text-slate-700">Fecha real de término<input type="date" required value={obraDetailsForm.fecha_termino_real} onChange={event => setObraDetailsForm({ ...obraDetailsForm, fecha_termino_real: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Motivo de cierre<textarea required rows={3} value={obraDetailsForm.motivo_cierre} onChange={event => setObraDetailsForm({ ...obraDetailsForm, motivo_cierre: event.target.value })} placeholder="Ej. Recepción conforme, término anticipado, cierre administrativo" className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm" /></label></div>}</section>{errorMsg && <p className="rounded-lg bg-rose-50 p-2 text-xs font-semibold text-rose-700">{errorMsg}</p>}<div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setShowObraDetails(false)} className="rounded-xl bg-slate-100 px-4 py-2.5 text-xs font-bold text-slate-700">Cancelar</button><button disabled={modalLoading} type="submit" className="rounded-xl bg-blue-900 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{modalLoading ? 'Guardando…' : 'Guardar ficha'}</button></div></form>
          </div>
        </div>
      )}
            ...
            <form
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  // Mantener la relación con el presupuesto para Lookahead / Last Planner y sus recursos APU.
                  if (newObraForm.presupuesto_enlazado && createdObra) {
                    try {
                      await supabase.from('obra_presupuestos').upsert({
                        empresa: user?.empresa || null,
                        obra_nombre: createdObra.nombre,
                        presupuesto_id: newObraForm.presupuesto_enlazado
                      }, { onConflict: 'obra_nombre' });
                    } catch (relationError) {
                      console.warn('No fue posible guardar la relación obra-presupuesto:', relationError.message);
                    }
                  }
