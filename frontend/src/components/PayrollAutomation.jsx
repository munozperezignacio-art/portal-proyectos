...
export default function PayrollAutomation({ user, personal, centrosGestion = [], indicadores, onEmit }) {
  ...
  const load = useCallback(async () => {
    ...
    const start = `${period}-01`, endDate = new Date(`${period}-01T12:00:00`); endDate.setMonth(endDate.getMonth() + 1); endDate.setDate(0);
    const [attendanceResult, runResult] = await Promise.all([
      supabase.from('asistencia_personal').select('*').eq('empresa', user.empresa).gte('fecha_marcacion', start).lte('fecha_marcacion', endDate.toISOString().slice(0, 10)),
  ...
  const save = async (close = false) => {
    ...
    const allocationCleanup = await supabase.from('obra_liquidaciones').delete().eq('nomina_id', payroll.id).not('nomina_item_id', 'is', null);
    if (allocationCleanup.error) {
      if (close) await supabase.from('rrhh_nominas_mensuales').update({ estado: 'Borrador' }).eq('id', payroll.id);
      setStatus('Borrador');
      setMessage(`No fue posible preparar la redistribución de la nómina: ${allocationCleanup.error.message}`);
      return;
    }
    const itemCleanup = await supabase.from('rrhh_nomina_items').delete().eq('nomina_id', payroll.id);
    if (itemCleanup.error) {
      if (close) await supabase.from('rrhh_nominas_mensuales').update({ estado: 'Borrador' }).eq('id', payroll.id);
      setStatus('Borrador');
      setMessage(`No fue posible actualizar el detalle de la nómina: ${itemCleanup.error.message}`);
      return;
    }
    ...
    if (close) {
      const allocationResult = await supabase.rpc('distribuir_nomina_a_obras', { p_nomina_id: payroll.id });
      if (allocationResult.error) {
        await supabase.from('rrhh_nominas_mensuales').update({ estado: 'Borrador' }).eq('id', payroll.id);
        setStatus('Borrador');
        setMessage(`La nómina fue guardada, pero no pudo cerrarse ni distribuirse entre las obras: ${allocationResult.error.message}`);
        return;
      }
      allocationSummary = allocationResult.data;
    }
    setStatus(header.estado);
    setSavedItems(items);
    setMessage(close
      ? `Nómina cerrada. ${allocationSummary?.imputaciones_obras || allocationSummary?.imputaciones_creadas || 0} imputaciones entre obras y ${allocationSummary?.imputaciones_partidas || 0} imputaciones por partida${allocationSummary?.trabajadores_sin_destino ? `; ${allocationSummary.trabajadores_sin_destino} trabajador(es) quedaron sin obra` : ''}${allocationSummary?.imputaciones_sin_partida ? `; ${allocationSummary.imputaciones_sin_partida} imputación(es) permanecen en Gastos Generales por falta de avances de cuadrilla vinculables` : ''}.`
      : 'Borrador mensual guardado.');
  };
  ...
  return <div className="space-y-4">
    ...
    <p className="text-[10px] text-slate-500">Al cerrar la nómina, el costo mensual se distribuye primero entre obras por asistencia y asignaciones; luego entre partidas según los avances y las cuadrillas ejecutoras reportadas. Los casos sin evidencia suficiente quedan identificados en Gastos Generales para revisión.</p>
  </div>;
}
