...
export default function PayrollAutomation({ user, personal, centrosGestion = [], indicadores, onEmit }) {
  ...
  const save = async (close = false) => {
    ...
    const allocationCleanup = await supabase.from('obra_liquidaciones').delete().eq('nomina_id', payroll.id);
    ...
    setSavedItems(items);
    setMessage(close
      ? `Nómina cerrada. ${allocationSummary?.imputaciones_creadas || 0} imputaciones distribuidas automáticamente entre obras${allocationSummary?.trabajadores_sin_destino ? `; ${allocationSummary.trabajadores_sin_destino} trabajador(es) quedaron sin destino y requieren revisar sus asignaciones` : ''}.`
  ...
  return <div className="space-y-4">
    ...
    <p className="text-[10px] text-slate-500">Al cerrar la nómina, el costo mensual de cada trabajador se distribuye automáticamente entre las obras: primero por días con asistencia y, para días sin marcaje, por el historial vigente de asignaciones. Los casos sin destino quedan informados para revisión.</p>
  </div>;
}
