...
import { AlertTriangle, CalendarRange, CircleDollarSign, FlaskConical, ShieldCheck, TrendingUp, Wrench } from 'lucide-react';
...
export default function PredictiveScenarioPanel({ obra, user, cutoff, baselineFinish, bac, eac, spi, cpi, actualProgress, plannedProgress, advances = [], costs = [], criticalItems = [], incidents = [], equipment = [] }) {
  ...
  useEffect(() => { let active = true; const load = async () => { if (!obra?.nombre) return setQuality([]); const { data } = await supabase.from('calidad_no_conformidades').select('estado,clasificacion,fecha_reporte,created_at,fecha_cierre').eq('empresa', user?.empresa || obra?.empresa || 'Obraxis').eq('obra_nombre', obra.nombre); if (active) setQuality(data || []); }; load(); return () => { active = false; }; }, [obra?.empresa, obra?.nombre, user?.empresa]);
  const model = useMemo(() => {
