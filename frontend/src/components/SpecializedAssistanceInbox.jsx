...
export default function SpecializedAssistanceInbox({ user, obra, criticalPartidas = [], upcomingPartidas = [], equipmentAvailability = [], incidents = [], personal = [], onNavigate }) {
  ...
  return <section className="space-y-4">
    <div className="rounded-2xl bg-gradient-to-r from-slate-950 via-indigo-950 to-blue-950 p-5 text-white"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-blue-200">Asistencia especializada</p><h4 className="mt-1 text-lg font-black">Bandeja priorizada de la obra</h4><p className="mt-1 max-w-3xl text-xs text-blue-100">Consolida desviaciones, restricciones y compromisos abiertos. No modifica registros: cada acción conduce al módulo responsable.</p></div><button type="button" onClick={load} disabled={loading} className="flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-xs font-black hover:bg-white/20 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`}/>Actualizar</button></div></div>
