...
import { BriefcaseBusiness, Check, ClipboardList, Pencil, Plus, Trash2 } from 'lucide-react';
...
export default function SubcontractContractManager({ obra, user, subcontracts = [] }) {
  ...
  const load = useCallback(async () => {
    const [contractResult, itemResult, partResult] = await Promise.all([
    ...
    const error = contractResult.error || itemResult.error || partResult.error;
  ...
  const selectPart = partId => {
    ...
    setItemForm(current => ({ ...current, partida_id: partId, codigo: part?.codigo || '', descripcion: part?.partida || '', unidad: part?.unidad || '' }));
  };
  ...
  return <section className="space-y-4 rounded-3xl border bg-white p-5">
    ...
    <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
      <div className="space-y-2"><p className="text-[10px] font-black uppercase text-slate-500">Subcontratista</p>{subcontracts.map(sub => <button key={sub.id} onClick={() => { setSelectedSubId(String(sub.id)); setSelectedContractId(''); setFormOpen(false); }} className={`w-full rounded-xl border p-3 text-left ${String(sub.id) === String(selectedSubId) ? 'border-orange-400 bg-orange-50' : 'bg-white'}`}><p className="text-xs font-black">{sub.empresa_nombre}</p><p className="text-[10px] text-slate-500">{sub.rut_empresa}</p></button>)}{!subcontracts.length && <p className="rounded-xl bg-slate-50 p-4 text-xs text-slate-500">Primero registra el subcontratista en Acreditaciones.</p>}</div>
      <div className="space-y-3"><p className="text-[10px] font-black uppercase text-slate-500">Contratos, órdenes o servicios</p>{subContracts.map(contract => <button key={contract.id} onClick={() => openEdit(contract)} className={`w-full rounded-2xl border p-4 text-left ${String(contract.id) === String(selectedContractId) ? 'border-orange-400 bg-orange-50' : 'bg-slate-50'}`}><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase text-orange-700">{contract.codigo} · {contract.tipo_vinculo}</p><p className="font-black">{contract.nombre}</p><p className="text-[11px] text-slate-500">{money(contract.monto_contrato, contract.moneda)} · {contract.estado}</p></div><Pencil className="h-4 w-4 text-slate-400"/></div><div className="mt-3 flex flex-wrap gap-1">{contract.reporta_avances && <Tag>Avances</Tag>}{contract.reporta_asistencia && <Tag>Asistencia</Tag>}{contract.reporta_estados_pago && <Tag>Estados de pago</Tag>}{!contract.reporta_avances && !contract.reporta_asistencia && !contract.reporta_estados_pago && <Tag>Sin reportes</Tag>}</div></button>)}{selectedSub && !subContracts.length && <p className="rounded-2xl border border-dashed bg-slate-50 p-6 text-center text-xs text-slate-500">Sin ficha contractual. Puede ser un subcontratista pendiente o un vínculo que no requiere operación.</p>}</div>
    </div>
    {formOpen && <div className="rounded-2xl border border-orange-200 bg-orange-50 p-4"><form onSubmit={saveContract} className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><input required className={input} placeholder="Código contrato / OC" value={form.codigo} onChange={e => setForm({...form,codigo:e.target.value})}/><input required className={`${input} xl:col-span-2`} placeholder="Nombre o alcance" value={form.nombre} onChange={e => setForm({...form,nombre:e.target.value})}/><select className={input} value={form.tipo_vinculo} onChange={e => setForm({...form,tipo_vinculo:e.target.value,...defaultsForType(e.target.value)})}>{['Ejecución de partidas','Prestación de servicios','Arriendo','Suministro','Otro'].map(value => <option key={value}>{value}</option>)}</select><textarea className={`${input} md:col-span-2 xl:col-span-4`} placeholder="Descripción del alcance" value={form.descripcion || ''} onChange={e => setForm({...form,descripcion:e.target.value})}/><select className={input} value={form.moneda} onChange={e => setForm({...form,moneda:e.target.value})}>{['CLP','UF','USD'].map(value => <option key={value}>{value}</option>)}</select><input className={input} type="number" min="0" step="any" placeholder="Monto contratado" value={form.monto_contrato} onChange={e => setForm({...form,monto_contrato:e.target.value})}/><input className={input} type="date" value={form.fecha_inicio} onChange={e => setForm({...form,fecha_inicio:e.target.value})}/><input className={input} type="date" value={form.fecha_termino} onChange={e => setForm({...form,fecha_termino:e.target.value})}/><select className={input} value={form.estado} onChange={e => setForm({...form,estado:e.target.value})}>{['Borrador','Vigente','Suspendido','Terminado','Cerrado'].map(value => <option key={value}>{value}</option>)}</select><input className={input} placeholder="Responsable operacional" value={form.responsable_operacion_nombre || ''} onChange={e => setForm({...form,responsable_operacion_nombre:e.target.value})}/><input className={`${input} md:col-span-2`} type="email" placeholder="Correo operacional (distinto de acreditación)" value={form.responsable_operacion_email || ''} onChange={e => setForm({...form,responsable_operacion_email:e.target.value})}/><div className="flex flex-wrap gap-2 md:col-span-2 xl:col-span-4"><Toggle checked={form.reporta_avances} onChange={value => setForm({...form,reporta_avances:value})}>Reporta avances</Toggle><Toggle checked={form.reporta_asistencia} onChange={value => setForm({...form,reporta_asistencia:value})}>Reporta asistencia</Toggle><Toggle checked={form.reporta_estados_pago} onChange={value => setForm({...form,reporta_estados_pago:value})}>Presenta estados de pago</Toggle></div><textarea className={`${input} md:col-span-2 xl:col-span-4`} placeholder="Observaciones contractuales" value={form.observaciones || ''} onChange={e => setForm({...form,observaciones:e.target.value})}/><button disabled={busy} className="flex items-center justify-center gap-2 rounded-xl bg-emerald-700 py-3 text-xs font-black text-white md:col-span-2 xl:col-span-4"><Check className="h-4 w-4"/>{busy ? 'Guardando…' : 'Guardar ficha contractual'}</button></form>
      {selectedContract && <div className="mt-5 border-t border-orange-200 pt-4"><div className="mb-3 flex items-center justify-between"><div><h4 className="font-black">Partidas, servicios o recursos contratados</h4><p className="text-[11px] text-slate-500">Total de ítems: {money(calculatedTotal, selectedContract.moneda)}. El monto contractual prevalece si es global.</p></div><ClipboardList className="h-5 w-5 text-orange-700"/></div><div className="space-y-2">{selectedItems.map(item => <div key={item.id} className="grid items-center gap-2 rounded-xl bg-white p-3 md:grid-cols-[auto_1fr_auto_auto]"><span className="rounded-lg bg-slate-100 px-2 py-1 text-[9px] font-black">{item.tipo_item}</span><div><p className="text-xs font-black">{item.codigo ? `${item.codigo} · ` : ''}{item.descripcion}</p><p className="text-[10px] text-slate-500">{Number(item.cantidad).toLocaleString('es-CL')} {item.unidad || ''} × {money(item.precio_unitario, selectedContract.moneda)}</p></div><span className="text-xs font-black">{money(Number(item.cantidad) * Number(item.precio_unitario), selectedContract.moneda)}</span>{mayConfigure && <button onClick={() => removeItem(item)} className="rounded-lg p-2 text-rose-600 hover:bg-rose-50"><Trash2 className="h-4 w-4"/></button>}</div>)}{!selectedItems.length && <p className="rounded-xl bg-white p-4 text-center text-xs text-slate-500">Aún no hay partidas o servicios agregados.</p>}</div>{mayConfigure && <form onSubmit={addItem} className="mt-3 grid gap-2 rounded-xl bg-white p-3 md:grid-cols-2 xl:grid-cols-6"><select className={input} value={itemForm.tipo_item} onChange={e => setItemForm({...itemForm,tipo_item:e.target.value,partida_id:'',codigo:'',descripcion:'',unidad:'',reporta_avance:e.target.value === 'Partida'})}>{['Partida','Servicio','Arriendo','Suministro','Otro'].map(value => <option key={value}>{value}</option>)}</select>{itemForm.tipo_item === 'Partida' ? <select required className={`${input} xl:col-span-2`} value={itemForm.partida_id} onChange={e => selectPart(e.target.value)}><option value="">Partida de la obra</option>{parts.map(part => <option key={part.id} value={part.id}>{part.codigo ? `${part.codigo} · ` : ''}{part.partida}</option>)}</select> : <input required className={`${input} xl:col-span-2`} placeholder="Descripción del servicio o recurso" value={itemForm.descripcion} onChange={e => setItemForm({...itemForm,descripcion:e.target.value})}/>}<input className={input} placeholder="Unidad" value={itemForm.unidad} onChange={e => setItemForm({...itemForm,unidad:e.target.value})}/><input required className={input} type="number" min="0" step="any" placeholder="Cantidad" value={itemForm.cantidad} onChange={e => setItemForm({...itemForm,cantidad:e.target.value})}/><input className={input} type="number" min="0" step="any" placeholder="Precio unitario" value={itemForm.precio_unitario} onChange={e => setItemForm({...itemForm,precio_unitario:e.target.value})}/><button className="flex items-center justify-center gap-1 rounded-xl bg-orange-600 py-2.5 text-xs font-black text-white md:col-span-2 xl:col-span-6"><Plus className="h-4 w-4"/>Agregar al contrato</button></form>}</div>}
    </div>}
  </section>;
}
