...
export default function MachineryScreen(){
  ...
  return <Screen refreshing={state.loading} onRefresh={state.refresh}>
    ...
    <ErrorBox text={state.error}/>{state.loading&&!state.data.length?<Loading/>:<>
      {tab==='equipos'&&(data.equipment.length?data.equipment.map((item:Machinery)=><Card key={item.id}><View style={s.row}><View style={s.machine}><Ionicons name="construct" size={22} color={colors.orange}/></View><View style={s.flex}><Text style={s.name}>{item.tipo||'Equipo'} · {item.patente||'S/P'}</Text><Text style={s.meta}>{item.marca||'Sin marca'} · {item.obra_nombre||'Bodega / Sin asignar'}</Text></View><Badge tone={String(item.estado_equipo).toLowerCase().includes('mant')?'amber':'green'}>{item.estado_equipo||'Operativo'}</Badge></View></Card>):<Empty text="No hay equipos registrados."/>)}
      ...
      {tab==='estadisticas'&&<View style={s.grid}><Metric value={data.equipment.length} label="Equipos"/><Metric value={stats.available} label="Disponibles"/><Metric value={stats.hours.toLocaleString('es-CL')} label="Horas registradas"/><Metric value={stats.failures} label="Fallas"/><Metric value={stats.maintenance} label="Mantenciones"/></View>}
    </>}</Screen>;
}
...
const s=StyleSheet.create({actions:{flexDirection:'row',gap:8,flexWrap:'wrap'},row:{flexDirection:'row',alignItems:'center',gap:10},machine:{width:43,height:43,borderRadius:13,backgroundColor:'#FFF4E8',alignItems:'center',justifyContent:'center'},flex:{flex:1},name:{fontSize:14,fontWeight:'900',color:colors.ink},meta:{fontSize:11,color:colors.muted,lineHeight:16},dot:{width:12,height:12,borderRadius:6},grid:{gap:10},value:{fontSize:29,fontWeight:'900',color:colors.blue}});
