...
CREATE INDEX IF NOT EXISTS bodega_movimientos_validacion_idx
  ON public.bodega_movimientos (empresa, estado_validacion, fecha DESC);
...
CREATE TABLE IF NOT EXISTS public.compras_conciliaciones (
  ...
  numero_oc TEXT, proveedor_rut TEXT, proveedor_nombre TEXT, obra_nombre TEXT,
  ...
  monto_oc NUMERIC(16,2) NOT NULL DEFAULT 0,
  monto_recepcion NUMERIC(16,2) NOT NULL DEFAULT 0,
  monto_factura NUMERIC(16,2) NOT NULL DEFAULT 0,
  monto_nota_credito NUMERIC(16,2) NOT NULL DEFAULT 0,
  diferencia_monto NUMERIC(16,2) NOT NULL DEFAULT 0,
  diferencia_cantidad NUMERIC(16,4) NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS compras_conciliaciones_empresa_idx
  ON public.compras_conciliaciones (empresa, estado, created_at DESC);
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.estado_validacion = 'Pendiente' AND NEW.estado_validacion IN ('Validado','Rechazado') THEN
    NEW.validado_por := COALESCE((auth.jwt()->>'email'), NEW.validado_por);
    NEW.validado_at := NOW();
  END IF;
