...
export function buildExecutiveReportHtml({ schedule, company, selected, advances, attendance, costs, nonConformities, prevention, parts, equipmentUse = [], failures = [], maintenance = [], personnel = [], periodDays = 7 }) {
  const periodStart = new Date(Date.now() - periodDays * day);
  const cards = selected.map(name => {
    const workParts = parts.filter(item => item.obra_nombre === name);
  ...
  const enrichedHtml = specialty ? html.replace('<h2 style="font-size:14px;margin:20px 8px 9px">Desempeño por obra</h2>', `${specialty}<h2 style="font-size:14px;margin:20px 8px 9px">Desempeño por obra</h2>`) : html;
  return { html: enrichedHtml, indicators:{ obras:selected.length, reportes:totalReports, bac, pv, ev, ac, spi, cpi, sv, cv, eac, dias_desviacion:scheduleVarianceDays, nc_abiertas:totalNc, equipos_con_uso:equipmentIds.size, horas_equipos:equipmentHours, fallas_abiertas:openFailures, mantenciones_periodo:maintenance.length, personal_asignado:assignedPersonnel, personal_sin_asignar:unassignedPersonnel }, cards };
}
