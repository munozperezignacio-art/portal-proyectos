...
create or replace function public.obraxis_actor_can_access_company(target_company text) returns boolean language sql stable security definer set search_path=public,auth as $$
...
$$;
create or replace function public.obraxis_actor_can_manage_company(target_company text) returns boolean language sql stable security definer set search_path=public,auth as $$
...
$$;
revoke all on function public.obraxis_actor_can_access_company(text) from public,anon;
revoke all on function public.obraxis_actor_can_manage_company(text) from public,anon;
grant execute on function public.obraxis_actor_can_access_company(text) to authenticated;
grant execute on function public.obraxis_actor_can_manage_company(text) to authenticated;

create or replace function public.obraxis_sync_usuario_auth() returns trigger language plpgsql security definer set search_path=public,auth,extensions as $$
...
create trigger trg_usuarios_sync_auth before insert or update of correo,contrasena on public.usuarios for each row execute function public.obraxis_sync_usuario_auth();
...
create policy usuarios_select_empresa on public.usuarios for select to authenticated using (public.obraxis_actor_can_access_company(empresa));
create policy usuarios_insert_empresa on public.usuarios for insert to authenticated with check (public.obraxis_actor_can_manage_company(empresa));
create policy usuarios_update_empresa on public.usuarios for update to authenticated using (public.obraxis_actor_can_manage_company(empresa)) with check (public.obraxis_actor_can_manage_company(empresa));
create policy usuarios_delete_empresa on public.usuarios for delete to authenticated using (public.obraxis_actor_can_manage_company(empresa));
