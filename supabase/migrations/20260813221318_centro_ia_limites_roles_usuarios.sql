-- Cierre de gobierno IA: lÃ­mites mensuales y autorizaciÃ³n por rol/usuario.
...
comment on column public.ia_config_empresas.limites_funcion is
  'Por funciÃ³n: presupuesto_mensual_usd, max_ejecuciones_usuario_mes y roles_autorizados.';
...
begin
  if p_auth_user_id is null then
    raise exception 'La ejecuciÃ³n de IA requiere un usuario autenticado.' using errcode='P0001';
  end if;
  ...
  if not v_config.habilitada then
    raise exception 'La inteligencia artificial estÃ¡ deshabilitada para esta empresa.' using errcode='P0001';
  end if;
  if coalesce((v_config.funciones->>p_funcion)::boolean,false) is false then
    raise exception 'Esta funciÃ³n de inteligencia artificial no estÃ¡ habilitada para la empresa.' using errcode='P0001';
  end if;
  ...
  if jsonb_array_length(v_roles)>0 and not exists (
    ...
    raise exception 'Tu rol no estÃ¡ autorizado para utilizar esta funciÃ³n de IA.' using errcode='P0001';
  end if;
  if v_limite_usuario ? 'habilitado' and not coalesce((v_limite_usuario->>'habilitado')::boolean,true) then
    raise exception 'El uso de IA estÃ¡ deshabilitado para este usuario.' using errcode='P0001';
  end if;
  ...
  if v_config.bloquear_al_limite and v_config.presupuesto_mensual_usd>0 and v_consumido+greatest(p_reserva_usd,0)>v_config.presupuesto_mensual_usd then
    raise exception 'Se alcanzÃ³ el presupuesto mensual de IA de la empresa.' using errcode='P0001';
  end if;
  if coalesce(v_presupuesto_funcion,0)>0 and v_consumido_funcion+greatest(p_reserva_usd,0)>v_presupuesto_funcion then
    raise exception 'Se alcanzÃ³ el presupuesto mensual de esta funciÃ³n de IA.' using errcode='P0001';
  end if;
  if coalesce(v_presupuesto_usuario,0)>0 and v_consumido_usuario+greatest(p_reserva_usd,0)>v_presupuesto_usuario then
    raise exception 'Se alcanzÃ³ el presupuesto mensual de IA de este usuario.' using errcode='P0001';
  end if;
  if coalesce(v_max_funcion_usuario,0)>0 and v_ejecuciones_usuario_funcion>=v_max_funcion_usuario then
    raise exception 'Se alcanzÃ³ el lÃ­mite mensual de ejecuciones para esta funciÃ³n.' using errcode='P0001';
  end if;
  if coalesce(v_max_usuario,0)>0 and v_ejecuciones_usuario>=v_max_usuario then
    raise exception 'Se alcanzÃ³ el lÃ­mite mensual de ejecuciones de IA de este usuario.' using errcode='P0001';
  end if;
