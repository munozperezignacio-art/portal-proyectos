...
begin
  ...
  from (values
    ...
    ('Programacion','Actualización de programación','Semanal',coalesce((new.paquetes->>'programacion')::boolean,false)),
    ('Estado de pago','Estado de pago','Mensual',coalesce((new.paquetes->>'estados_pago')::boolean,false)),
    ('Prevencion','Informe de prevención',coalesce(new.periodicidad->>'prevencion','Mensual'),coalesce((new.paquetes->>'prevencion')::boolean,false)),
