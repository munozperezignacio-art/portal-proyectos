...
create index if not exists mandante_integraciones_contrato_idx
  on public.mandante_integraciones (contrato_id, created_at desc);
create index if not exists mandante_integraciones_fuente_idx
  on public.mandante_integraciones (fuente_tabla, fuente_id);
...
comment on table public.mandante_integraciones is
  'Trazabilidad inmutable de snapshots contractuales sincronizados desde empresas Obraxis vinculadas.';
