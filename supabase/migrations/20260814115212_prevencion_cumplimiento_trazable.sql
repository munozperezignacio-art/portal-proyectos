...
alter table public.prevencion_cumplimiento_registros add column if not exists empresa text;
alter table public.prevencion_cumplimiento_asignaciones
  drop constraint if exists prevencion_cumplimiento_asignaciones_usuario_id_fkey,
  add constraint prevencion_cumplimiento_asignaciones_usuario_id_fkey foreign key (usuario_id) references public.usuarios(id) on delete restrict,
  drop constraint if exists prevencion_cumplimiento_asignaciones_formulario_id_fkey,
  add constraint prevencion_cumplimiento_asignaciones_formulario_id_fkey foreign key (formulario_id) references public.prevencion_formularios(id) on delete restrict,
  drop constraint if exists prevencion_cumplimiento_asignaciones_frecuencia_check,
  add constraint prevencion_cumplimiento_asignaciones_frecuencia_check check (frecuencia in ('Diario', 'Semanal', 'Mensual'));
...
create index if not exists prevencion_cumplimiento_asignaciones_empresa_idx on public.prevencion_cumplimiento_asignaciones (empresa);
create index if not exists prevencion_cumplimiento_registros_empresa_idx on public.prevencion_cumplimiento_registros (empresa);
create or replace function public.validar_asignacion_cumplimiento()
returns trigger language plpgsql set search_path = public as $$
declare v_empresa_usuario text; v_empresa_formulario text;
begin
  if tg_op = 'UPDATE' and new.empresa is null and new.usuario_id is null and new.formulario_id is null then return new; end if;
...
create trigger trg_validar_asignacion_cumplimiento before insert or update on public.prevencion_cumplimiento_asignaciones
...
begin
  select empresa into v_empresa from public.prevencion_cumplimiento_asignaciones where id = new.asignacion_id;
  if v_empresa is null then raise exception 'La asignación de cumplimiento no existe'; end if;
...
create trigger trg_empresa_registro_cumplimiento before insert or update on public.prevencion_cumplimiento_registros
...
create policy "cumplimiento_asignaciones_empresa" on public.prevencion_cumplimiento_asignaciones for all to authenticated
using (private.obraxis_actor_can_access_company(empresa)) with check (private.obraxis_actor_can_access_company(empresa));
create policy "cumplimiento_registros_empresa" on public.prevencion_cumplimiento_registros for all to authenticated
using (private.obraxis_actor_can_access_company(empresa)) with check (private.obraxis_actor_can_access_company(empresa));
