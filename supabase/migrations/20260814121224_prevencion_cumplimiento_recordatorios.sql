...
alter table public.prevencion_cumplimiento_asignaciones
  ...
  add constraint prevencion_cumplimiento_dia_semana_check check (dia_semana is null or dia_semana between 1 and 5),
  drop constraint if exists prevencion_cumplimiento_dia_mes_check,
  add constraint prevencion_cumplimiento_dia_mes_check check (dia_mes is null or dia_mes between 1 and 28);
...
comment on column public.prevencion_cumplimiento_asignaciones.hora_limite is 'Hora local de Chile desde la cual se notifica si el formulario sigue pendiente.';
comment on column public.prevencion_cumplimiento_asignaciones.dia_semana is 'Día ISO de aviso semanal: lunes=1 a viernes=5.';
comment on column public.prevencion_cumplimiento_asignaciones.dia_mes is 'Día de aviso mensual, limitado a 1..28 para existir en todos los meses.';
...
begin
  if new.notificar_pendiente and new.empresa is not null then
    insert into public.notificaciones_reglas (
      ...
      new.empresa, 'Cumplimiento preventivo pendiente',
      'prevencion_cumplimiento_pendiente', 'Prevención',
      'Avisa al responsable cuando el formulario asignado no fue completado al vencimiento configurado.',
      '[]'::jsonb, '[]'::jsonb, '[]'::jsonb, true, true, 'Diaria',
...
drop trigger if exists trg_asegurar_regla_cumplimiento_preventivo on public.prevencion_cumplimiento_asignaciones;
...
select distinct a.empresa, 'Cumplimiento preventivo pendiente',
  'prevencion_cumplimiento_pendiente', 'Prevención',
  'Avisa al responsable cuando el formulario asignado no fue completado al vencimiento configurado.',
  '[]'::jsonb, '[]'::jsonb, '[]'::jsonb, true, true, 'Diaria',
  jsonb_build_object('alcance_tipo', 'todas', 'origen', 'cumplimiento_preventivo'),
