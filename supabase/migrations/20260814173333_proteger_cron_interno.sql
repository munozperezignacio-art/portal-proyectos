...
begin
  if not exists (
    select 1 from vault.decrypted_secrets where name = 'obraxis_cron_secret'
  ) then
...
as $$
  select nullif(p_secret, '') is not null
...
select cron.schedule(
  'obraxis-detectar-reportes-pendientes','*/15 * * * *',
  $cron$
    select net.http_post(
